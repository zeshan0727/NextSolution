...
__attribute__((constructor)) static void MGInit(void) {
    @autoreleasepool {
        ...
        MGLog(@"ModuleGlassRuntime 1.0.6 loaded SpringBoard=%@ prefsEnabled=%d", NSBundle.mainBundle.bundleIdentifier, MGBoolPreference(@"CCModuleBackgroundsEnabled", YES));
        MGLog(@"Passive slider-aware overlay runtime process=%@ pid=%d dlopen=%p moduleClass=%@ contentClass=%@", NSProcessInfo.processInfo.processName, getpid(), handle, moduleClass, contentClass);
