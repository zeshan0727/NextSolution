...
struct DashboardView: View {
            ...
            if store.transactions.isEmpty {
                ...
                VStack(spacing: 0) {
                    ForEach(Array(store.transactions.prefix(6).enumerated()), id: \.element.id) { index, transaction in
                        TransactionRow(transaction: transaction, currencyCode: store.currencyCode)
    ...
    private var allTimeBalance: Decimal {
        store.transactions.reduce(Decimal.zero) { result, item in
            result + (item.type == .income ? item.amount : -item.amount)
        }
    }
}
