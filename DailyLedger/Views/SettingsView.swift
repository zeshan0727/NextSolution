...
struct SettingsView: View {
    ...
    @State private var smsAutoImportEnabled = true
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section {
                    Toggle(isOn: $smsAutoImportEnabled) {
                        SettingsRow(
                            title: "SMS Auto Import",
                            subtitle: "Allow the RootHide add-on to record bank SMS",
                            icon: "message.fill",
                            color: AppTheme.teal
                        )
                    }
            ...
            .onAppear {
                ...
                smsAutoImportEnabled = store.settings.smsAutoImportEnabled
            }
            .onChange(of: selectedCurrency) { store.updateCurrency($0) }
            .onChange(of: smsAutoImportEnabled) { store.updateSMSAutoImport($0) }
            .fileExporter(
