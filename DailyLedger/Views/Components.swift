...
struct TransactionRow: View {
    ...
    var body: some View {
        HStack(spacing: 13) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(transaction.details.isEmpty ? transaction.category : transaction.details)
                    .font(.body.weight(.semibold))
                    .foregroundStyle(.primary)
                    .lineLimit(1)
                Text(transaction.details.isEmpty ? DisplayFormat.day.string(from: transaction.date) : transaction.category)
                    .font(.caption)
            ...
            Text(amountText)
                .font(.subheadline.bold())
                .foregroundStyle(transaction.type == .income ? AppTheme.green : AppTheme.red)
                .lineLimit(1)
                .minimumScaleFactor(0.7)
        }
        .padding(.vertical, 5)
    ...
    private var amountText: String {
        ...
        return prefix + DisplayFormat.currency(transaction.amount, code: currencyCode)
    }
}
