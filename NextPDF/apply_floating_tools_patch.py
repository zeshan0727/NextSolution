...
'''
        ...
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                // Keep the PDF at full width. Editing controls float above it instead
                // of permanently shrinking the page on compact iPhone screens.
                PDFKitEditorView(document: document, model: model)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                    .background(Color(uiColor: .secondarySystemBackground))
...
'''
        text = replace_once(text, old_toggle, new_toggle, "toolbar floating-tools toggle")

        old_status = '''            } else if model.hasSelectedAnnotation {
                Label(model.selectedAnnotationName, systemImage: "selection.pin.in.out")
                    .foregroundStyle(.blue)
                    .font(.caption.weight(.semibold))
            } else {
'''
        new_status = '''            } else if model.hasSelectedAnnotation {
...
'''
        text = replace_once(text, old_status, new_status, "annotation gesture status hint")

        old_sidebar_header = '''private struct RobustEditorSidebar: View {
    @ObservedObject var model: PDFEditorModel
    let onEditText: () -> Void
'''
        new_sidebar_header = '''private struct RobustEditorSidebar: View {
    @ObservedObject var model: PDFEditorModel
    let onCollapse: () -> Void
    let onEditText: () -> Void
'''
        text = replace_once(text, old_sidebar_header, new_sidebar_header, "sidebar collapse callback")

        old_sidebar_title = '''                VStack(alignment: .leading, spacing: 5) {
...
'''
        new_sidebar_title = '''                HStack(alignment: .top, spacing: 8) {
...
'''
        text = replace_once(text, old_sidebar_title, new_sidebar_title, "floating palette collapse button")

        old_text_section = '''                RobustSidebarSection(title: "TEXT") {
...
'''
        new_text_section = '''                RobustSidebarSection(title: "TEXT") {
                    RobustSidebarButton("Edit Text", icon: "character.cursor.ibeam", primary: true, action: onEditText)
...
'''
        text = replace_once(text, old_text_section, new_text_section, "disable unavailable text tools")
...
''',
            "transparent floating sidebar interior",
        )

        button_anchor = '''private struct RobustSidebarButton: View {'''
        anchor_index = text.find(button_anchor)
        if anchor_index == -1:
            raise RuntimeError("sidebar button declaration not found")
...
''',
            "disabled sidebar button appearance",
        )
        text = prefix + suffix

        WORKSPACE.write_text(text, encoding="utf-8")
        print("Applied NextPDF floating collapsible editing palette and UI cleanup")
        return 0
    except Exception as exc:
        print(f"Floating tools patch failed: {exc}", file=sys.stderr)
        return 1
