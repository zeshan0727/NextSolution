...
def upsert_marked(text, start, end, block, anchor):
    pattern=re.compile(r'^[ \t]*<!-- '+re.escape(start)+r' -->.*?^[ \t]*<!-- '+re.escape(end)+r' -->', re.M|re.S)
    if pattern.search(text):
        return pattern.sub(block, text, count=1)
    pos=text.find(anchor)
    if pos < 0: raise SystemExit(f'Anchor missing: {anchor}')
    line_start=text.rfind('\n',0,pos)+1
    return text[:line_start]+block+'\n\n'+text[line_start:]

changed=[]
p=Path('index.html'); text=p.read_text()
text=upsert_marked(text,f'{SLUG}_RECENT_CARD_START',f'{SLUG}_RECENT_CARD_END',recent,'<!-- NEXT_HOME_LOCK_RECENT_CARD_START -->')
p.write_text(text); changed.append(str(p))

p=Path('tutorials.html'); text=p.read_text()
text=upsert_marked(text,f'{SLUG}_TUTORIAL_CARD_START',f'{SLUG}_TUTORIAL_CARD_END',tutorial_card,'<!-- NEXT_HOME_LOCK_TUTORIAL_CARD_START -->')
p.write_text(text); changed.append(str(p))
...
pattern=re.compile(r'^[ \t]*<url><loc>https://nextsolution\.cc/'+re.escape(ARTICLE)+r'</loc>.*?</url>', re.M)
if pattern.search(text):
