...
# Verify repaired Gmail connector handling and original due-time extensions.
grep -q 'nextGmailConnectionInvalidated' NextReminder/Sources/GmailConnection.swift
grep -q 'connectorExpired' NextReminder/Sources/FileSharing.swift
grep -q 'Connect Gmail Account' NextReminder/Sources/FileSharing.swift
grep -q 'GmailConnectionStore.shared.invalidate' NextReminder/Sources/EmailAutomationCore.swift
grep -q 'originalDueDate' NextReminder/Sources/CompletedFilterActions.swift
grep -q "first/original due date and time" NextReminder/Sources/CompletedFilterActions.swift
grep -q 'NextReminder-iOS/1.2.4' NextReminder/Sources/FileSharing.swift
...
plutil -extract CFBundleShortVersionString raw "$APP/Info.plist" | grep -qx '1.2.4'
plutil -extract CFBundleVersion raw "$APP/Info.plist" | grep -qx '8'
...
mkdir -p Payload
cp -R "$APP" Payload/
ditto -c -k --sequesterRsrc --keepParent Payload NextReminder-v1.2.4-unsigned.ipa
cp NextReminder-v1.2.4-unsigned.ipa NextReminder-v1.2.4-unsigned.tipa
shasum -a 256 NextReminder-v1.2.4-unsigned.tipa > NextReminder-v1.2.4-unsigned.tipa.sha256
unzip -t NextReminder-v1.2.4-unsigned.tipa >/dev/null

echo "Next Reminder v1.2.4 build and verification completed."
