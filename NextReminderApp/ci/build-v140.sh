...
# Verify notification delivery fixes.
grep -q 'effectiveOffsets.insert(.atTime)' NextReminder/Sources/Services.swift
grep -q 'An exact-time alert is mandatory' NextReminder/Sources/Services.swift
grep -q 'content.interruptionLevel = .timeSensitive' NextReminder/Sources/Services.swift
grep -q 'content.relevanceScore = 1.0' NextReminder/Sources/Services.swift
grep -q 'checkDeliverySettings' NextReminder/Sources/Services.swift
grep -q 'Notification Delivery Needs Attention' NextReminder/Sources/RootReminders.swift
grep -q 'Open Notification Settings' NextReminder/Sources/RootReminders.swift

# Verify Gmail persistence and reconnect behaviour.
grep -q 'manualDisconnectKey' NextReminder/Sources/GmailConnection.swift
grep -q 'wasManuallyDisconnected' NextReminder/Sources/GmailConnection.swift
grep -q 'temporary server' NextReminder/Sources/GmailConnection.swift
grep -q 'Gmail needs reconnection' NextReminder/Sources/GmailConnection.swift
grep -q 'Reconnect Gmail' NextReminder/Sources/GmailConnection.swift
grep -q 'Gmail is not connected on this installation' NextReminder/Sources/GmailConnection.swift
! grep -q 'UserDefaults.standard.removeObject(forKey: key).*let emailAddress' NextReminder/Sources/GmailConnection.swift

# Verify retained features.
...
plutil -extract CFBundleShortVersionString raw "$APP/Info.plist" | grep -qx '1.3.10'
plutil -extract CFBundleVersion raw "$APP/Info.plist" | grep -qx '20'
file "$APP/NextReminder" | grep -q 'arm64'

rm -rf Payload
mkdir -p Payload
cp -R "$APP" Payload/
ditto -c -k --sequesterRsrc --keepParent Payload NextReminder-v1.3.10-unsigned.ipa
cp NextReminder-v1.3.10-unsigned.ipa NextReminder-v1.3.10-unsigned.tipa
shasum -a 256 NextReminder-v1.3.10-unsigned.tipa > NextReminder-v1.3.10-unsigned.tipa.sha256
unzip -t NextReminder-v1.3.10-unsigned.tipa >/dev/null

echo "Next Reminder v1.3.10 build and verification completed."
