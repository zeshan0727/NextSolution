...
# Verify notification repair.
grep -q 'var effectiveOffsets = reminder.alertOffsets' NextReminder/Sources/Services.swift
...
grep -q 'let categoryNames = Dictionary' NextReminder/Sources/Services.swift
grep -q 'await NotificationManager.shared.schedule' NextReminder/Sources/Services.swift
grep -q 'status == .notDetermined' NextReminder/Sources/Services.swift

# Verify routine behavior remains intact.
...
plutil -extract CFBundleShortVersionString raw "$APP/Info.plist" | grep -qx '1.3.7'
plutil -extract CFBundleVersion raw "$APP/Info.plist" | grep -qx '17'
file "$APP/NextReminder" | grep -q 'arm64'

rm -rf Payload
mkdir -p Payload
cp -R "$APP" Payload/
ditto -c -k --sequesterRsrc --keepParent Payload NextReminder-v1.3.7-unsigned.ipa
cp NextReminder-v1.3.7-unsigned.ipa NextReminder-v1.3.7-unsigned.tipa
shasum -a 256 NextReminder-v1.3.7-unsigned.tipa > NextReminder-v1.3.7-unsigned.tipa.sha256
unzip -t NextReminder-v1.3.7-unsigned.tipa >/dev/null

echo "Next Reminder v1.3.7 build and verification completed."
