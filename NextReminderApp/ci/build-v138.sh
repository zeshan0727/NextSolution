...
# Verify Gmail disconnect alerts and proactive health checking.
grep -q 'final class GmailConnectionAlertManager' NextReminder/Sources/GmailConnection.swift
grep -q 'Gmail disconnected' NextReminder/Sources/GmailConnection.swift
grep -q 'Reconnect Gmail' NextReminder/Sources/GmailConnection.swift
grep -q 'final class GmailConnectionHealthMonitor' NextReminder/Sources/GmailConnection.swift
grep -q 'v1/connectors/gmail/\\(encoded)/status' NextReminder/Sources/GmailConnection.swift
grep -q 'GmailConnectionIssueStore.shared.save' NextReminder/Sources/GmailConnection.swift
grep -q 'nextGmailConnectionInvalidated' NextReminder/Sources/RootReminders.swift
grep -q 'Gmail Disconnected' NextReminder/Sources/RootReminders.swift
grep -q 'GmailConnectionHealthMonitor.shared.checkNow' NextReminder/Sources/App.swift
grep -q 'GmailConnectionAlertManager.shared.installCategory' NextReminder/Sources/App.swift
grep -q 'GmailConnectionStore.isDisconnectionMessage' NextReminder/Sources/EmailAutomationCore.swift
grep -q 'reason: FileShareError.connectorExpired.localizedDescription' NextReminder/Sources/FileSharing.swift

# Verify notification and routine features remain intact.
grep -q 'effectiveOffsets.insert(.atTime)' NextReminder/Sources/Services.swift
grep -q 'UNTimeIntervalNotificationTrigger(timeInterval: 1' NextReminder/Sources/Services.swift
...
# Verify scheduler health endpoint patch syntax.
pushd ../NextReminderScheduler >/dev/null
node apply-v138.js
node --check server.js
grep -q "connectors/gmail/:connectorID/status" server.js
grep -q "version: '1.3.8'" server.js
popd >/dev/null
...
plutil -extract CFBundleShortVersionString raw "$APP/Info.plist" | grep -qx '1.3.8'
plutil -extract CFBundleVersion raw "$APP/Info.plist" | grep -qx '18'
file "$APP/NextReminder" | grep -q 'arm64'

rm -rf Payload
mkdir -p Payload
cp -R "$APP" Payload/
ditto -c -k --sequesterRsrc --keepParent Payload NextReminder-v1.3.8-unsigned.ipa
cp NextReminder-v1.3.8-unsigned.ipa NextReminder-v1.3.8-unsigned.tipa
shasum -a 256 NextReminder-v1.3.8-unsigned.tipa > NextReminder-v1.3.8-unsigned.tipa.sha256
unzip -t NextReminder-v1.3.8-unsigned.tipa >/dev/null

echo "Next Reminder v1.3.8 build and verification completed."
