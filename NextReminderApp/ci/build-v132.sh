...
# Verify cost controls, response diagnostics, fixed placeholders, and visual-only retry.
...
grep -q 'incomplete_details' NextReminder/Sources/XPostGenerator.swift
grep -q 'responseRefusal' NextReminder/Sources/XPostGenerator.swift
...
plutil -extract CFBundleShortVersionString raw "$APP/Info.plist" | grep -qx '1.3.2'
plutil -extract CFBundleVersion raw "$APP/Info.plist" | grep -qx '12'
...
mkdir -p Payload
cp -R "$APP" Payload/
ditto -c -k --sequesterRsrc --keepParent Payload NextReminder-v1.3.2-unsigned.ipa
cp NextReminder-v1.3.2-unsigned.ipa NextReminder-v1.3.2-unsigned.tipa
shasum -a 256 NextReminder-v1.3.2-unsigned.tipa > NextReminder-v1.3.2-unsigned.tipa.sha256
unzip -t NextReminder-v1.3.2-unsigned.tipa >/dev/null

echo "Next Reminder v1.3.2 build and verification completed."
