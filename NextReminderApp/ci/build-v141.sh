...
# Verify Send Email Now for every reminder detail/review screen.
grep -q 'Send Email Now' NextReminder/Sources/Detail.swift
grep -q 'emailActionCard(reminder)' NextReminder/Sources/Detail.swift
grep -q 'showEmailSetup' NextReminder/Sources/Detail.swift
grep -q 'EmailAutomationSettingsView' NextReminder/Sources/Detail.swift
grep -q 'Available for one-time, completed, repeating and Routine reminders' NextReminder/Sources/Detail.swift
grep -q 'func sendNow' NextReminder/Sources/EmailAutomationCore.swift
grep -q 'testOnly: true' NextReminder/Sources/EmailAutomationCore.swift
grep -q 'UIApplication.shared.open' NextReminder/Sources/EmailAutomationCore.swift
grep -q 'GmailConnectionStore.isDisconnectionMessage' NextReminder/Sources/EmailAutomationCore.swift

# Verify retained notification, Gmail, routine and X generator fixes.
...
plutil -extract CFBundleShortVersionString raw "$APP/Info.plist" | grep -qx '1.3.11'
plutil -extract CFBundleVersion raw "$APP/Info.plist" | grep -qx '21'
file "$APP/NextReminder" | grep -q 'arm64'

rm -rf Payload
mkdir -p Payload
cp -R "$APP" Payload/
ditto -c -k --sequesterRsrc --keepParent Payload NextReminder-v1.3.11-unsigned.ipa
cp NextReminder-v1.3.11-unsigned.ipa NextReminder-v1.3.11-unsigned.tipa
shasum -a 256 NextReminder-v1.3.11-unsigned.tipa > NextReminder-v1.3.11-unsigned.tipa.sha256
unzip -t NextReminder-v1.3.11-unsigned.tipa >/dev/null

echo "Next Reminder v1.3.11 build and verification completed."
