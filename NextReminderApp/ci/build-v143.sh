...
# Verify quick reminder URL importer and confirmation UI.
grep -q 'nextQuickReminderImportResult' NextReminder/Sources/QuickReminderImport.swift
grep -q 'nextreminder' NextReminder/Sources/QuickReminderImport.swift
grep -q 'quick-add' NextReminder/Sources/QuickReminderImport.swift
grep -q 'HourlyRepeatStore.shared.save' NextReminder/Sources/QuickReminderImport.swift
grep -q 'ReminderCategory.routines.id' NextReminder/Sources/QuickReminderImport.swift
grep -q 'EmailAutomationSettings.load' NextReminder/Sources/QuickReminderImport.swift
grep -q 'QuickReminderImporter.handle' NextReminder/Sources/App.swift
grep -q 'Quick Reminder' NextReminder/Sources/RootReminders.swift
...
plutil -extract CFBundleShortVersionString raw "$APP/Info.plist" | grep -qx '1.3.13'
...
strings -a "$APP/NextReminder" | grep -q 'NextReminder.QuickImportResult'
strings -a "$APP/NextReminder" | grep -q 'quick-add'

rm -rf Payload
mkdir -p Payload
cp -R "$APP" Payload/
ditto -c -k --sequesterRsrc --keepParent Payload NextReminder-v1.3.13-unsigned.ipa
cp NextReminder-v1.3.13-unsigned.ipa NextReminder-v1.3.13-unsigned.tipa
shasum -a 256 NextReminder-v1.3.13-unsigned.tipa > NextReminder-v1.3.13-unsigned.tipa.sha256
unzip -t NextReminder-v1.3.13-unsigned.tipa >/dev/null

echo "Next Reminder v1.3.13 build and verification completed."
