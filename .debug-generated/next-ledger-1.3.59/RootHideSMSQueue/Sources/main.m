...
static NSString *const kDaemonVersion = @"2.2.1";
...
static NSString *StringFromDecodedObject(id object) {
...
static void ScanMessages(BOOL forceRecent) {
    ...
    NSInteger approvedMessageLimit = manualRequested ? 15 : 10;
    NSInteger databaseRowLimit = manualRequested ? 15 : 250;
    const char *query =
        "SELECT m.ROWID, COALESCE(m.guid, ''), m.text, m.attributedBody, m.date, COALESCE(h.id, '') "
        "FROM message m LEFT JOIN handle h ON m.handle_id = h.ROWID "
        "WHERE m.is_from_me = 0 AND (m.text IS NOT NULL OR m.attributedBody IS NOT NULL) "
