    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Dates") {
                    DatePicker("Assigned", selection: $draft.assignedDate, displayedComponents: [.date, .hourAndMinute])
                    DatePicker("Due", selection: $draft.dueDate, in: draft.assignedDate..., displayedComponents: [.date, .hourAndMinute])
                }

                Section("Time & Price") {
                    Stepper(value: $draft.targetMinutes, in: 0...100_000, step: 15) {
                        LabeledContent("Target time", value: JobRecord.timeText(minutes: draft.targetMinutes))
                    }
                    ...
                    if draft.hasActualTime {
                        Stepper(value: $draft.actualMinutes, in: 0...100_000, step: 15) {
                            LabeledContent("Actual time", value: JobRecord.timeText(minutes: draft.actualMinutes))
                        }
                    }
                    TextField("Job price", value: $draft.price, format: .number.precision(.fractionLength(2)))
            ...
            .onAppear {
                if draft.clientName.isEmpty { draft.clientName = store.settings.companyName }
                if draft.jobType.isEmpty { draft.jobType = store.settings.jobTypes.first?.name ?? "Other" }
            }
