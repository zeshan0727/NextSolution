...
@property(nonatomic, assign) CGPoint lastFinalCenter;
...
static UIFont *LGTSafeFont(NSString *fontName, CGFloat size, NSInteger weight, NSInteger style, UIFont *fallback) {
    ...
    if ([name isEqualToString:@"Original"]) {
        ...
        if ([name isEqualToString:@"System Rounded"]) {
            UIFontDescriptor *d = [descriptor fontDescriptorWithDesign:UIFontDescriptorSystemDesignRounded];
            if (d) descriptor = d;
        } else if ([name isEqualToString:@"System Serif / New York"]) {
            UIFontDescriptor *d = [descriptor fontDescriptorWithDesign:UIFontDescriptorSystemDesignSerif];
            if (d) descriptor = d;
        } else if ([name isEqualToString:@"System Monospaced"]) {
            UIFontDescriptor *d = [descriptor fontDescriptorWithDesign:UIFontDescriptorSystemDesignMonospaced];
            if (d) descriptor = d;
        }
        if (traits) {
            UIFontDescriptor *d = [descriptor fontDescriptorWithSymbolicTraits:(descriptor.symbolicTraits | traits)];
            if (d) descriptor = d;
        }
        ...
        if (font && traits) {
            UIFontDescriptor *d = [font.fontDescriptor fontDescriptorWithSymbolicTraits:(font.fontDescriptor.symbolicTraits | traits)];
            if (d) font = [UIFont fontWithDescriptor:d size:MAX(size, 1.0)];
        }
    }
...
static BOOL LGTPointsClose(CGPoint a, CGPoint b) {
    return fabs(a.x - b.x) < 1.0 && fabs(a.y - b.y) < 1.0;
}

static void LGTPrepareLabelForNativeLayout(UILabel *label) {
    LGTLabelState *state = objc_getAssociatedObject(label, LGTLabelStateAssociationKey);
    if (!state) return;

    if (state.hasAppliedPosition && LGTPointsClose(label.center, state.lastFinalCenter)) {
        label.center = CGPointMake(label.center.x - state.lastOffsetX, label.center.y - state.lastOffsetY);
    }
    ...
    if (state.attributedText) label.attributedText = state.attributedText;
    else label.text = state.text;
...
static void LGTCaptureNativeState(UILabel *label) {
    LGTLabelState *state = LGTStateForLabel(label);
    state.transform = label.transform;
    state.font = label.font ?: state.font;
    state.textColor = label.textColor ?: state.textColor;
    state.textAlignment = label.textAlignment;
    state.text = label.text;
    state.attributedText = label.attributedText;
    state.shadowColor = label.shadowColor;
...
#define S(KEY,DEF) ((s=[prefs stringForKey:(KEY)]) && s.length ? s : (DEF))
    ...
    gCustomTimeEnabled = B(@"customTimeEnabled", YES);
    gTimeScale = D(@"timeScale", 1.0, 0.5, 2.5);
    gTimeColor = S(@"timeColor", @"#FFFFFF");
    gTimeOffsetX = D(@"timeOffsetX", 0.0, -150.0, 150.0);
    gTimeOffsetY = D(@"timeOffsetY", 0.0, -150.0, 150.0);
    gTimeAlignment = I(@"timeAlignment", 0);
    gTimeFont = S(@"timeFont", @"Original");
    gTimeFontWeight = I(@"timeFontWeight", 3);
    gTimeStyle = I(@"timeStyle", 0);
    gTimeShadowEnabled = B(@"timeShadowEnabled", NO);
    gTimeShadowColor = S(@"timeShadowColor", @"#000000");
    gTimeShadowOpacity = D(@"timeShadowOpacity", 0.45, 0.0, 1.0);
    gTimeShadowRadius = D(@"timeShadowRadius", 2.0, 0.0, 20.0);
    gTimeShadowOffsetX = D(@"timeShadowOffsetX", 0.0, -20.0, 20.0);
    gTimeShadowOffsetY = D(@"timeShadowOffsetY", 1.0, -20.0, 20.0);

    gCustomDateEnabled = B(@"customDateEnabled", YES);
    gDateScale = D(@"dateScale", 1.0, 0.5, 2.5);
    gDateColor = S(@"dateColor", @"#FFFFFF");
    gDateOffsetX = D(@"dateOffsetX", 0.0, -150.0, 150.0);
    gDateOffsetY = D(@"dateOffsetY", 0.0, -150.0, 150.0);
    gDateAlignment = I(@"dateAlignment", 0);
    gDateFont = S(@"dateFont", @"Original");
    gDateFontWeight = I(@"dateFontWeight", 3);
    gDateStyle = I(@"dateStyle", 0);
    gDateFormat = S(@"dateFormat", @"system");
    gCustomDateFormat = S(@"customDateFormat", @"EEEE, MMMM d");
    gDateShadowEnabled = B(@"dateShadowEnabled", NO);
    gDateShadowColor = S(@"dateShadowColor", @"#000000");
    gDateShadowOpacity = D(@"dateShadowOpacity", 0.45, 0.0, 1.0);
    gDateShadowRadius = D(@"dateShadowRadius", 2.0, 0.0, 20.0);
    gDateShadowOffsetX = D(@"dateShadowOffsetX", 0.0, -20.0, 20.0);
    gDateShadowOffsetY = D(@"dateShadowOffsetY", 1.0, -20.0, 20.0);

    gIconEnabled = B(@"iconEnabled", YES);
    gIconName = S(@"iconName", @"sparkles");
    gIconSize = D(@"iconSize", 22.0, 10.0, 80.0);
    gIconColor = S(@"iconColor", @"#FFFFFF");
    gAnchorTarget = I(@"anchorTarget", 0);
    gIconPosition = I(@"iconPosition", 1);
    gIconOffsetX = D(@"iconOffsetX", 0.0, -120.0, 120.0);
    gIconOffsetY = D(@"iconOffsetY", 0.0, -120.0, 120.0);
    gShadowEnabled = B(@"shadowEnabled", YES);
    gShadowColor = S(@"shadowColor", @"#000000");
    gShadowOpacity = D(@"shadowOpacity", 0.45, 0.0, 1.0);
    gShadowRadius = D(@"shadowRadius", 2.0, 0.0, 20.0);
    gShadowOffsetX = D(@"shadowOffsetX", 0.0, -20.0, 20.0);
    gShadowOffsetY = D(@"shadowOffsetY", 1.0, -20.0, 20.0);
#undef B
#undef D
#undef I
#undef S
...
static void LGTReloadCallback(CFNotificationCenterRef c, void *o, CFStringRef n, const void *obj, CFDictionaryRef info) {
    LGTLoadPrefs();
    LGTRequestRelayout();
}
...
static void LGTCollectLabels(UIView *view, NSMutableArray<UILabel *> *labels) {
    ...
    for (UIView *subview in view.subviews) LGTCollectLabels(subview, labels);
}
...
static NSInteger LGTTimeScore(UILabel *label) {
    ...
    NSString *probe = LGTProbe(label);
    if ([probe containsString:@"time"]) score += 1000;
    if ([probe containsString:@"clock"]) score += 800;
    if ([probe containsString:@"date"]) score -= 500;
    NSUInteger digits = 0;
    for (NSUInteger i=0;i<text.length;i++) if ([NSCharacterSet.decimalDigitCharacterSet characterIsMember:[text characterAtIndex:i]]) digits++;
    if (digits >= 3 && ([text containsString:@":"] || [text containsString:@"."])) score += 1500;
    if (digits >= 3) score += 250;
    return score;
}
...
static NSInteger LGTDateScore(UILabel *label) {
    ...
    for (NSUInteger i=0;i<text.length;i++) if ([NSCharacterSet.letterCharacterSet characterIsMember:[text characterAtIndex:i]]) letters++;
    if (letters >= 3) score += 500;
...
static void LGTResolveLabels(UIView *container, UILabel **timeOut, UILabel **dateOut) {
    ...
    NSInteger ts = NSIntegerMin, ds = NSIntegerMin;
    for (UILabel *label in labels) {
        NSInteger s = LGTTimeScore(label);
        if (s > ts) { ts = s; time = label; }
    }
    for (UILabel *label in labels) {
        if (label == time) continue;
        NSInteger s = LGTDateScore(label);
        if (s > ds) { ds = s; date = label; }
    }
    if (!time && labels.count) {
        [labels sortUsingComparator:^NSComparisonResult(UILabel *a, UILabel *b) {
            return a.font.pointSize > b.font.pointSize ? NSOrderedAscending : (a.font.pointSize < b.font.pointSize ? NSOrderedDescending : NSOrderedSame);
        }];
        time = labels.firstObject;
        if (labels.count > 1) date = labels[1];
    }
...
static void LGTApplyAttributedStyle(UILabel *label, UIFont *font, UIColor *color, NSInteger style, BOOL isDate) {
    NSString *text = label.text ?: label.attributedText.string ?: @"";
    if (isDate && style == 5) text = text.uppercaseString;
    else if (isDate && style == 6) text = text.lowercaseString;
    ...
    NSInteger outline = isDate ? 7 : 5;
    if (style == outline) {
        attrs[NSStrokeColorAttributeName] = color ?: label.textColor;
        attrs[NSStrokeWidthAttributeName] = @(-2.5);
    }
...
static void LGTApplyLayerShadow(UILabel *label, BOOL enabled, UIColor *color, CGFloat opacity, CGFloat radius, CGFloat x, CGFloat y, NSInteger style, BOOL isDate, UIColor *textColor) {
    if (enabled) {
        ...
        label.layer.shadowOffset = CGSizeMake(x,y);
...
static void LGTApplyPosition(UILabel *label, LGTLabelState *state, CGFloat x, CGFloat y) {
    CGPoint nativeCenter = label.center;
    label.center = CGPointMake(nativeCenter.x + x, nativeCenter.y + y);
    state.lastOffsetX = x;
    state.lastOffsetY = y;
    state.lastFinalCenter = label.center;
    state.hasAppliedPosition = YES;
}

static void LGTApplyTime(UILabel *label) {
    if (!label) return;
    LGTLabelState *state = LGTStateForLabel(label);
    if (!gEnabled || !gCustomTimeEnabled) { LGTRestoreCapturedState(label,state); return; }
    ...
    label.textAlignment = LGTAlignmentForValue(gTimeAlignment, state.textAlignment);
    label.transform = CGAffineTransformScale(state.transform, gTimeScale, gTimeScale);
    LGTApplyPosition(label, state, gTimeOffsetX, gTimeOffsetY);
...
static void LGTApplyDate(UILabel *label) {
    if (!label) return;
    LGTLabelState *state = LGTStateForLabel(label);
    if (!gEnabled || !gCustomDateEnabled) { LGTRestoreCapturedState(label,state); return; }
    if (gDateFormatter) {
    ...
    label.font = font;
    label.textColor = color;
    label.textAlignment = LGTAlignmentForValue(gDateAlignment, state.textAlignment);
    label.transform = CGAffineTransformScale(state.transform, gDateScale, gDateScale);
    LGTApplyPosition(label, state, gDateOffsetX, gDateOffsetY);
...
static UIImage *LGTSystemImage(NSString *name) {
...
static CGRect LGTIconFrame(CGRect anchor, CGFloat size, NSInteger position) {
    CGFloat spacing = 8.0, x = CGRectGetMaxX(anchor)+spacing, y = CGRectGetMidY(anchor)-size/2.0;
    if (position == 0) { x = CGRectGetMinX(anchor)-size-spacing; y = CGRectGetMidY(anchor)-size/2.0; }
    else if (position == 2) { x = CGRectGetMidX(anchor)-size/2.0; y = CGRectGetMinY(anchor)-size-spacing; }
    else if (position == 3) { x = CGRectGetMidX(anchor)-size/2.0; y = CGRectGetMaxY(anchor)+spacing; }
...
static void LGTApplyIcon(UIView *container, UILabel *time, UILabel *date) {
    ...
    if (!icon) {
        ...
        objc_setAssociatedObject(container,LGTIconAssociationKey,icon,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    icon.image = LGTSystemImage(gIconName);
    icon.tintColor = gIconUIColor ?: UIColor.whiteColor;
    icon.layer.shadowOpacity = gShadowEnabled ? (float)gShadowOpacity : 0.0f;
    icon.layer.shadowRadius = gShadowRadius;
    icon.layer.shadowOffset = CGSizeMake(gShadowOffsetX,gShadowOffsetY);
    icon.layer.shadowColor = (gIconShadowUIColor ?: UIColor.blackColor).CGColor;
    ...
    icon.frame = LGTIconFrame(frame,gIconSize,gIconPosition);
    [container bringSubviewToFront:icon];
}

static void LGTPrepareContainerForNativeLayout(UIView *container) {
    UILabel *time=nil,*date=nil;
    LGTResolveLabels(container,&time,&date);
    if (time) LGTPrepareLabelForNativeLayout(time);
    if (date) LGTPrepareLabelForNativeLayout(date);
}

static void LGTApplyContainer(UIView *container) {
    if (!container) return;
    [gKnownContainers addObject:container];
    UILabel *time=nil,*date=nil;
    LGTResolveLabels(container,&time,&date);
    if (!time) {
        UIImageView *icon = objc_getAssociatedObject(container,LGTIconAssociationKey);
    ...
    LGTCaptureNativeState(time);
    if (date) LGTCaptureNativeState(date);
    LGTApplyTime(time);
    if (date) LGTApplyDate(date);
...
static void LGTHookedLayout(id self, SEL _cmd) {
    if ([self isKindOfClass:UIView.class]) LGTPrepareContainerForNativeLayout((UIView *)self);
    if (gOriginalLayout) gOriginalLayout(self,_cmd);
    if ([self isKindOfClass:UIView.class]) LGTApplyContainer((UIView *)self);
}

static Class LGTResolveContainerClass(void) {
    NSArray<NSString *> *candidates = @[@"SBFLockScreenDateView",@"CSDateView",@"SBFLockScreenDateSubtitleView"];
    for (NSString *name in candidates) {
        Class cls = NSClassFromString(name);
        if (cls && class_getInstanceMethod(cls,@selector(layoutSubviews))) return cls;
    }
    return Nil;
}
...
%ctor {
    @autoreleasepool {
        ...
        Class cls = LGTResolveContainerClass();
        if (cls) MSHookMessageEx(cls,@selector(layoutSubviews),(IMP)LGTHookedLayout,(IMP *)&gOriginalLayout);
    }
}
