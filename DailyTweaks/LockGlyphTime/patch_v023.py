...
if old_spec not in prefs_src:
    raise SystemExit("Specifier method not found")
...
- (void)viewDidLoad {
    ...
    UIImage *logo=[UIImage imageNamed:@"NextLockIcon" inBundle:[NSBundle bundleForClass:self.class] compatibleWithTraitCollection:nil];
    UIImageView *iv=[[UIImageView alloc] initWithImage:logo];
    iv.frame=CGRectMake(24,24,72,72); iv.layer.cornerRadius=16; iv.clipsToBounds=YES; iv.contentMode=UIViewContentModeScaleAspectFit; [header addSubview:iv];
    UILabel *title=[[UILabel alloc] initWithFrame:CGRectMake(112,28,190,34)]; title.text=@"NextLock"; title.font=[UIFont systemFontOfSize:28 weight:UIFontWeightBold]; [header addSubview:title];
...
- (void)applyNow {
    [self lgtNotify];
    UIAlertController *a=[UIAlertController alertControllerWithTitle:@"NextLock" message:@"Changes applied." preferredStyle:UIAlertControllerStyleAlert];
    [a addAction:[UIAlertAction actionWithTitle:@"Done" style:UIAlertActionStyleDefault handler:nil]];
    [self presentViewController:a animated:YES completion:nil];
}
- (void)resetKeys:(NSArray<NSString *> *)keys {
    NSUserDefaults *p=[[NSUserDefaults alloc] initWithSuiteName:LGTPrefsDomain];
    for(NSString *k in keys)[p removeObjectForKey:k];
...
print("Generated RuntimeV023.xm and branded NextLock preference controller")
