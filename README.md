...
- Optional RootHide 2.0+ SMS importer for automatic local bank-message recording
...
4. Extract the artifact, open `DailyLedger-1.2.0.tipa` on the iPhone, and choose TrollStore.
...
The build artifact also includes `DailyLedgerSMSImport-1.0.0-roothide.deb`. Install it using Sileo or Zebra inside RootHide Bootstrap 2.0 or later. The launch daemon runs as the `mobile` user, reads only new incoming messages after installation, and records messages that contain a supported QAR transaction pattern. It does not use the network.

The importer extracts the amount, vendor, transaction time, and type. It stores the complete message as the description and applies the editable rules in **Daily Ledger → Settings → Vendor Category Rules**. Disable processing at any time with **Settings → SMS Auto Import**.

The add-on intentionally starts at the newest SMS database row on first launch, so it does not import old message history or create a large number of duplicates.
